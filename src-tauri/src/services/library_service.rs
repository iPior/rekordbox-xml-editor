...
pub fn open_rekordbox_xml(path: &str) -> AppResult<LoadedLibrary> {
    // Parse into normalized structs immediately so the UI never manipulates raw XML.
...
pub fn save_rekordbox_xml(request: &SaveLibraryRequest) -> AppResult<SaveLibraryResult> {
    ...
    // Write-then-rename keeps saves atomic in the common case and prevents partial files.
